...
export async function load({ params, fetch, setHeaders }) {
	...
	let [pageData, listData, featuredData] = await Promise.all([
		fetch(`/api/blog/${slug}.json`),
		fetch(`/api/listContent.json`),
		fetch(`/api/listContentFeatured.json`),
	])
	...
	if (featuredData.status > 400) {
		throw error(featuredData.status, await featuredData.text());
	}
	setHeaders({
	...
	return {
		...
		featured: (await featuredData.json()).slice(0, 3),
		slug,
		REPO_URL
	};
	// } catch (err) {
